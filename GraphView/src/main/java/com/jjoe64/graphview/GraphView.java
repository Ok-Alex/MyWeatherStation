...
abstract public class GraphView extends LinearLayout {
				...
				if ((event.getAction() & MotionEvent.ACTION_MOVE) == MotionEvent.ACTION_MOVE) {
					if (scrollingStarted) {
						if (lastTouchEventX != 0) {
                            float f = event.getX() - lastTouchEventX;
							onMoveGesture(f);
                            if (syncGraph != null) {
                                syncGraph.getGraphViewContentView().onMoveGesture(f);
                            }
                        }
    ...
    public void setSyncGraphView(GraphView syncGraph) {
        this.syncGraph = syncGraph;
    }

    public boolean isContainsSeries(GraphViewSeries graphViewSeries) {
        return graphSeries.contains(graphViewSeries);
    }
	...
	private String[] generateHorlabels(float graphwidth) {
		...
		DateFormat simpleDateFormat = new java.text.SimpleDateFormat("hh:mm");

		for (int i = 0; i <= numLabels; i++) {
			labels[i] = formatLabel(min + ((max-min)*i/numLabels), true);
			//labels[i] = SimpleDateFormat.getTimeInstance().format(new Date());
			double value = min + ((max - min) * i / numLabels);
			mCalendar.setTimeInMillis((long) value);
			labels[i] = simpleDateFormat.format(mCalendar.getTime());
		}
		return labels;
	}
