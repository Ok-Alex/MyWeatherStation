...
public class GraphViewSeries {
	...
	public GraphViewSeries(GraphViewDataInterface[] values) {
		...
		this.values = doLowPassFilter(values);
        checkValueOrder();
	}
	...
	public void appendData(GraphViewDataInterface value, boolean scrollToEnd) {
		...
		values = doLowPassFilter(newValues);
	...
	public GraphViewDataInterface get(int index) {
		return values[index];
	}
	...
	private GraphViewDataInterface[] doLowPassFilter(GraphViewDataInterface[] input) {
//		GraphView.GraphViewData[] output = new GraphView.GraphViewData[input.length];
//		for ( int i = 0; i < input.length; i++ ) {
//			output[i].valueY = output[i].valueY + ALPHA * (input[i].valueY - output[i].valueY);
//		}
//		return output;
		for (int j = 0, m = 4; j < m; j++) {
			for (int i = 1, l = input.length - 1; i < l; i++) {
				input[i].setY(0.5 * (0.5 * (input[i - 1].getY() + input[i + 1].getY()) + input[i].getY()));
			}
		}
		return input;
	}
